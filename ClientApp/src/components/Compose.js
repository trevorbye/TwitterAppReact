...
import { pollObjToString, pollInit } from "./utils/twitter-poll-util";
import { getHandles } from './utils/twitter-tweet-util';
...
export class Compose extends Component {
  ...
  async componentDidMount() {
    ...
    let handles = await getHandles(this.props.msalConfig);
  ...
  async createTweet() {
    ...
    try {
      ...
      if (error.response.status === 400) {
        ...
        this.setState({
          errorMessage:
            "One of your images exceeds the size limit for requests within this application (22mb). Additionally, individual images must be <=5mb in size.",
          processingTweet: false,
        });
      }
    }
  ...
  async dropdownChange(event) {
    ...
    let res = await axios.get(
      `${baseUrl}api/get-public-schedule?handle=${event.target.value}`,
      authHeaders
    );
  ...
  pollOnChange(newPoll) {
    ...
    console.log("Compose state changed");
  }
  getPolls = () => {
    return pollObjToString(this.state.poll);
  };
  ...
  render() {
    return (
      <div className="col-md-6">
        ...
        <p className="mb-3">
          Select a handle, compose a tweet, and choose a time when the status
          will be posted to the account. The request will be routed to the
          handle owner for approval. If the handle owner has made their tweet
          schedule public, click the calendar to view it.
        </p>

        <div className="input-group mb-3">
          <div className="input-group-prepend">
            <label className="input-group-text" htmlFor="handleSelect">
              Twitter handle
            </label>
            {this.state.publicHandleEvents.length > 0 ? (
              <span className="input-group-text">
                <i
                  className="far fa-calendar-alt fa-lg public-calendar"
                  onClick={() => this.toggleCalendarModal()}
                ></i>
                <sup className="ml-1">
                  {" "}
                  <b className="new-text">NEW</b>
                </sup>
              </span>
            ) : (
              <span className="input-group-text">
                <i
                  className="far fa-calendar-alt fa-lg public-calender-disabled"
                  disabled
                ></i>
                <sup className="ml-1">
                  {" "}
          ...
          <select
            ...
            value={this.state.selectedHandle}
          >
            {this.state.globalHandles.map((handle, idx) => (
        ...
        <div className="input-group mb-3">
          <label>
            Leave either date <strong>or</strong> time blank to post the tweet
            as soon as it's approved.
          </label>
          <label className="sr-only" htmlFor="tweet-date">
            Schedule tweet date
          </label>
          <label className="sr-only" htmlFor="tweet-time">
            Schedule tweet time
          </label>
          <div className="input-group-prepend">
          ...
          <div className="input-group mb-3" ng-hide="imageFileList.length >= 4">
            <div className="custom-file">
              <input
                ...
                multiple
              />
              <label className="custom-file-label" htmlFor="images-file-input">
                Add images
              </label>
            </div>
          </div>
        ...
        <ul className="list-group list-group-horizontal mb-3">
          {this.state.imageFileList.map((base64, index) => (
            <TweetImageBlock
              base64={base64}
              idx={index}
        ...
        this.state.imageFileList.length <= 4 ? (
          <div className="mb-3">
            ...
            {this.state.processingTweet && (
              <span>
                <div
                  className="spinner-border text-success ml-2"
                  id="wait-tweet"
                  role="status"
                  aria-hidden="true"
                ></div>
        ...
        {this.state.imageFileList.length > 4 && (
          <div className="alert alert-danger" role="alert">
            <strong>Oops!</strong> You exceeded the maximum of{" "}
